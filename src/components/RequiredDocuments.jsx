...
import freestyle from "../../public/pdfFiles/Ազատոճի.pdf";
import powerofattorney from "../../public/pdfFiles/Լիզաորագիր.pdf";
    ...
    const documents = [
        {
            ...
            files: [
                { name: "Ազատոճի ձևաթուղթ", file: freestyle },
                { name: "Լիազորագիր", file: powerofattorney },
            ],
        },
    ];
            ...
            <div className="flex mt-4 mb-8">
                <div className="w-1/3 bg-gray-50 p-4 rounded-2xl shadow-lg">
                    <ul className="list-none p-0 m-0">
                        {menuItems.map((item, index) => (
                            <li key={item.path} className="border-b border-gray-200 last:border-b-0">
                                <Link
                                    to={item.path}
                ...
                <div className="w-2/3 bg-white rounded-2xl shadow-lg p-6 mr-4">
                    {documents.map((report) => (
                        <div key={report.id} className="mb-4 border border-gray-200 rounded-lg overflow-hidden">
                            <button
                                    ...
                                    <table className="min-w-full table-auto border-collapse">
                                        ...
                                        <tbody>
                                            {report.files.map((file, index) => (
                                                <tr key={index}>
                                                    <td className="border-b px-4 py-2 font-semibold">{file.name}</td>
